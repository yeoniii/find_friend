	...
	<table>
		<thead>
			...
			<td style="width: 120px;text-align:center;">No</td>
				<td style="width: 120px;text-align:center;">받은사람</td>
		...
		<tbody>
			...
			<c:choose>
				<c:when test="${not empty sessionScope.researchInfo.size()}">
				<p>${sessionScope.find.opt } : ${sessionScope.find.text } 에 관한 검색 정보입니다.</p><br>
				<% int i=1; %>
					<c:forEach items="${ sessionScope.researchInfo }" var="list">
						<tr>
						<td style="text-align:center"><%=i %></td>
							<td style="text-align:center">${list.member.memberName }(${list.receiveId})</td>
							<td style="text-align:center"><a
								href="/MiniProject/mini/msg/viewSend?message_no=${list.messageNo }">${list.title }</a></td>
